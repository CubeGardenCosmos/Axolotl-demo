; =============================================================================
; 《Axolotl 美西螈引擎特性巡礼》 — 舞台演出席（画面演出与图鉴）
;
; 本章展示的特性：
...
雪乃:这一章是舞台演出席，重点看图鉴与演出指令。 -Vbeep_heroine.wav;
...
雪乃:两张 CG 已登记解锁，稍后可到「鉴赏（图鉴占位）」查看。 -Vbeep_heroine.wav;

unlockBgm:bgm/fairy_dance.ogg -name=Fairy Dance;
雪乃:BGM 图鉴也记录了解锁曲目。 -Vbeep_heroine.wav;
...
wait:200;
雪乃:例如 playVideo 视频播放、getUserInput 文本输入，当前运行时先记入日志，后续 Phase 补齐。 -Vbeep_heroine.wav;
playVideo:video/demo.webm;
getUserInput:player_name -title=输入你的名字 -buttonText=确定;

showVars;
雪乃:showVars 会在屏幕一角叠加变量表，供剧本作者现场排错。 -Vbeep_heroine.wav;
